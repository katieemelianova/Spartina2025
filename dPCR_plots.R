...
results %<>% mutate(copies_per_ul_converted=(copies_per_ul * (12/2) * dilution_factor))
...
phylo_rennes <- subset_taxa(phylo_rennes, Family %in% c("Mitochondria", "Chloroplast") | Order %in% c("Mitochondria", "Chloroplast"))

# get relative abundance
phylo_rennes_prop <- transform_sample_counts(phylo_rennes, function(otu) otu/sum(otu))

phylo_rennes_prop %>% ps_melt() %>% 
